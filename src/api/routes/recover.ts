...
export default (app: Router) => {
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const recoverServiceInstance = Container.get(RecoverService);
        const response = await recoverServiceInstance.ConfirmResetPasswordToken(req.body.token);
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const mailStatus = await mailerInstance.SendRecoveryPasswordEmail(req.body.email);
